...
class BuyerController(val bService: BuyerService) {
     ...
     *
     * ModelAttribute를 사용하면 각각 view template의 고유 기능을 사용하여
     * id name value 값을 자동으로 채워넣는 기능을 만들수 있다
     *
     * thymleaf template 를 사용할때
     * form tag에 model 에 담긴 object를 지정해두고
     * 각 input box에서는 field 속성으로 해당 맴버변수(요소,속성)을 지정해두면
     * template 엔진이 rendering을 수행하면서
     * input 에 필요한 요소들을 적절하게 생성해준다
     */
     ...
     * 여기에서는 userid가 필요없지만 Pathvariable로 설정해줘야한다
     */
